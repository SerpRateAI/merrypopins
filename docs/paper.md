...
In addition to pop-in detection, `merrypopins` also includes a statistical analysis suite. This suite provides functions to automatically calculate stress-strain curves, calculate precursor statistics (i.e., are there events occuring prior to a pop-in such as yielding), and temporal statistics across pop-in events. These are both accessible in the library and in a no-code streamlit app.
