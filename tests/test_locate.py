...
def test_detect_popins_savgol_no_jump(simple_curve):
    df0 = detect_popins_savgol(simple_curve, window_length=11, polyorder=2, threshold=3.0)
    assert df0["popin_savgol"].sum() <= 3

def test_detect_popins_savgol_with_jump(curve_with_jump):
    df1 = detect_popins_savgol(curve_with_jump, window_length=11, polyorder=2, threshold=1.0)
...
def test_detect_popins_cnn_basic(curve_with_jump):
    df_cnn = detect_popins_cnn(curve_with_jump, window_size=20, epochs=2, threshold_multiplier=1.0)
...
def test_default_locate_combine(curve_with_jump):
    ...
    max_single = df_all[["popin_iforest","popin_cnn","popin_fd","popin_savgol"]].sum(axis=1).max()
